...
test('home is one direct visual decision instead of a long story', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto('/en');
  await expect(page.getByRole('heading', { name: 'How would you divide it?' })).toBeVisible();
  await expect(page.getByRole('img', { name: /Maya and Leo compare three items/i })).toBeVisible();
  await expect(page.getByRole('list', { name: 'Three steps' }).locator('li')).toHaveCount(3);
...
test('reduced motion and no-JS retain a complete first frame', async ({ browser }) => {
  ...
  await expect(page.getByRole('heading', { name: '你會怎麼分？' })).toBeVisible();
...
for (const locale of ['en', 'zh-TW']) {
  test(`${locale} mobile home stays compact`, async ({ page }) => {
